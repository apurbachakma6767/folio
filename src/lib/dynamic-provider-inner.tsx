...
/**
 * Actual Dynamic SDK provider — only imported on the client after mount
 * so `window` / localStorage are never touched during SSR.
 */
export default function DynamicProviderInner({ children }: { children: React.ReactNode }) {
  const logoUrl =
    typeof window !== 'undefined'
      ? `${window.location.origin}/logo.svg`
      : 'https://folio.bar/logo.svg';
